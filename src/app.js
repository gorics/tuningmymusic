...
import { toCSV, downloadFile } from "./util.js";
...
function registerRoutes() {
  ...
  registerRoute("/connect", () => {
    ...
    form.setAttribute("data-card", "");
    form.innerHTML = `
      <h2>OAuth settings</h2>
      <label>Spotify Client ID <input name="spotify" value="${config.spotify.clientId ?? ""}" required /></label>
      <label>Google Client ID <input name="google" value="${config.google.clientId ?? ""}" required /></label>
      <p class="hint">Redirect URIs:</p>
      <code>${config.spotify.redirectUri}</code><br/>
      <code>${config.google.redirectUri}</code>
      <fieldset>
        <legend>Persistence</legend>
        <p>Tokens are stored in sessionStorage by default. Provide a passphrase to enable encrypted localStorage persistence.</p>
    ...
    form.addEventListener("submit", (event) => {
      ...
      updateConfig({
        spotify: { clientId: formData.get("spotify") },
        google: { clientId: formData.get("google") },
      });
      showToast("OAuth settings saved");
    });

    setTimeout(() => attachPersistenceListeners(form), 0);

    container.append(spotifyCard, googleCard, form);
    return container;
  });
